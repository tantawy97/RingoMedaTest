...
namespace Client.EmailClient.Services
{
    public class ViewRender : IViewRender
    {
        ...
        public string Render<TModel>(string viewName, TModel model, ViewDataDictionary? viewData)
        {
            var actionContext = GetActionContext();
            var viewEngineResult = _viewEngine.FindView(actionContext, viewName, true);
            if (!viewEngineResult.Success)
                throw new InvalidOperationException($"View Not Found {viewName}");
            var view = viewEngineResult.View;
            ViewDataDictionary viewDataDict;
            if (viewData != null)
                viewDataDict = new ViewDataDictionary<TModel>(viewData) { Model = model };
            else
            {
                var metadataProvider = new EmptyModelMetadataProvider();
                var modelState = new ModelStateDictionary();
                viewDataDict = new ViewDataDictionary<TModel>(metadataProvider, modelState) { Model = model };
            }
